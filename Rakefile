...
# Compose a labeled line: the label carries the color, the message stays normal
# weight. `spaced` adds a blank line above. Mirrors cli/messages.rb in the
# ruby2d gem — this repo builds standalone and can't require that file, so the
# two are kept in step by hand.
def labeled(label, msg, spaced)
  "#{"\n" if spaced}#{label} #{msg}"
end

def error(msg, spaced: false)
  puts labeled('Error:'.error, msg, spaced)
end

def warning(msg, spaced: false)
  puts labeled('Warning:'.warning, msg, spaced)
end

# Print an `Error:` line to stderr and exit non-zero
def abort_error(msg, spaced: false)
  abort labeled('Error:'.error, msg, spaced)
end
...
def require_cmd!(cmd, error_msg = nil)
  ...
  abort_error(error_msg || "#{cmd} is not installed or not found in PATH")
end
...
def check_windows_build_tools!
  ...
  if runner.nil? || pkg_list.nil?
    ...
    abort_error 'git and cmake are required to build.'
  end
  ...
  unless answer&.downcase == 'y'
    abort_error 'git and cmake are required to build. Install the packages above and re-run.'
  end
  ...
  unless install_ok && still_missing.empty?
    ...
    abort_error "Couldn't install build tools automatically#{detail}.\n" \
...
task build: [:preflight, :sdl, :mruby] do
  ...
  if emscripten_available?
    ...
    warning 'Skipping WASM build — Emscripten not found (run `rake wasm` after sourcing emsdk_env.sh)', spaced: true
  end
  ...
  task build: [:download, :clean] do
    ...
    SDL_NAMES.each do |lib|
      unless Dir.exist?(File.join(SOURCES_DIR, lib))
        error "Missing #{lib}. Run `rake sdl:download` to download sources."
        exit 1
      end
    end
  ...
  namespace :mruby do
    ...
    task :build do
      ...
      unless Dir.exist?(mruby_src)
        error 'Missing mruby source. Run `rake mruby:download` to download.'
        exit 1
      end
      ...
      unless File.exist?(host_mrbc)
        error "Native mruby compiler not found (#{host_mrbc})."
